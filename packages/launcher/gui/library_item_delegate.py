...
class LibraryItemDelegate(QStyledItemDelegate):
	...
	def createEditor(self, parent, option, index):
		...
		template_id = index.model().index(index.row(), 0, index.parent()).data(Qt.UserRole + 19)
	...
	def paint(self, painter, option, index):
		...
		app_icon_width = 25
		rect_app_icon = QRect(
			rect_item.right() - pad_data - app_icon_width,
			rect_data.top() + pad_data,
			app_icon_width,
			app_icon_width
		)

		rect_icon_type = QRect(
			rect_item.left() + pad_data,
			rect_data.top() + int((rect_data.height()/3)/2) - int(self._icon_size/2),
			self._icon_size,
			self._icon_size
		)
