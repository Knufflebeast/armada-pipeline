...
# from armada import resource
# from armada import definitions
# from armada.path_maker import make_dirs
...
class ArmadaSetupPopup(QtWidgets.QDialog):
	"""Sets up user and/or shared data depending on type of setup process

	TODO: Process can be reused to update data or create new project mounts in the future

		Types of setups:
		- FULL:
			- Setup flow for the first person to use path as a mount point
			Status:
			- Mount point isn't configured and there is no structure setup
			Data being setup:
			- User config's ARMADA_MOUNT_PREFIX
			- Shared data's shared_settings.json structure value
			- Create and assign a Structure in shared_data

		-  MOUNT:
			- This will be the setup flow for all users after the first
			Status:
			- Mount point isn't configured, but the structure exists in shared_data
			Data being setup:
			- User config's ARMADA_MOUNT_PREFIX

		- STRUCTURE:
			- This is more of a debug situation where the mount point is configred but the structure has been deleted.
			Status:
			- Mount point is configured but structure doesn't exists in shared_data
			Data being setup:
			- Shared data's shared_settings.json structure value
			- Create and assign a Structure in shared_data
	"""
	...
	def __init__(self, setup=FULL):
		...
		"""
		super(ArmadaSetupPopup, self).__init__()
		...
		self.armada_root_path = armada.definitions.ROOT_PATH

		self.setWindowFlags(QtCore.Qt.FramelessWindowHint | QtCore.Qt.WindowStaysOnTopHint)
		self.setAttribute(QtCore.Qt.WA_DeleteOnClose)
		self.installEventFilter(self)
		self.setStyleSheet(armada.resource.style_sheet('setup'))
		...
		# GUI ------------------------------
		pixmap_banner = armada.resource.pixmap(name='banner_setup', scope='help')
		...
		self.cb_s0_welcome = QtWidgets.QCheckBox('Beta guidelines')
		self.cb_s0_welcome.setStyleSheet(self.cb_style_sheet)
		self.cb_s0_welcome.setEnabled(False)
		...
		self.btn_hint.setIcon(armada.resource.color_svg('help', 128, "#777777"))
		self.btn_hint.setStyleSheet(armada.resource.style_sheet('push_button_w_icon'))

		# Mount point setup
		self.lbl_mount_path = QtWidgets.QPushButton(armada.resource.color_svg('folder_pipeline_root', 1024, '#FFFFFF' ), '')
		self.lbl_mount_path.setText("Mount point directory:")
		self.lbl_mount_path.setStyleSheet(armada.resource.style_sheet('icon_label'))
		...
		# Maya path setup
		self.lbl_maya_path = QtWidgets.QPushButton(armada.resource.icon('maya', 'png'), '')
		self.lbl_maya_path.setText("Parent path of the '{}' directory:".format(self.maya_ver))
		self.lbl_maya_path.setStyleSheet(armada.resource.style_sheet('icon_label'))
		self.lbl_maya_path.hide()

		self.le_maya_path = QtWidgets.QLineEdit()
		self.le_maya_path.setText(maya_path)
		self.le_maya_path.hide()

		self.btn_maya_browse = QtWidgets.QPushButton("Browse")
		self.btn_maya_browse.setMinimumWidth(100)
		self.btn_maya_browse.hide()

		# Maya path setup
		self.lbl_blender_path = QtWidgets.QPushButton(armada.resource.icon('blender', 'png'), '')
		self.lbl_blender_path.setText("Parent path of the '{}' directory:".format(self.blender_ver))
		self.lbl_blender_path.setStyleSheet(armada.resource.style_sheet('icon_label'))
		self.lbl_blender_path.hide()

		self.le_blender_path = QtWidgets.QLineEdit()
		self.le_blender_path.setText(blender_path)
		self.le_blender_path.hide()

		self.btn_blender_browse = QtWidgets.QPushButton("Browse")
		self.btn_blender_browse.setMinimumWidth(100)
		self.btn_blender_browse.hide()

		# Hide mount elements if entering into structure
		...
		if self.setup == FULL or self.setup == STRUCTURE:
			# Structure setup
			self.lbl_items_title = QtWidgets.QLabel()
		...
		self.s0_welcome = QtCore.QState()
		self.s1_mount_setup = QtCore.QState()
		self.s2_structure_workflow = QtCore.QState()
		self.s3_structure_sel = QtCore.QState()
		self.s4_software = QtCore.QState()

		# Entry point for setup
		if self.setup == FULL:
			# Transitions
			self.trans_s0_s1 = self.s0_welcome.addTransition(self.btn_right.clicked, self.s1_mount_setup)
			self.trans_s1_s2 = self.s1_mount_setup.addTransition(self.btn_right.clicked, self.s2_structure_workflow)
			self.trans_s2_s3 = self.s2_structure_workflow.addTransition(self.btn_right.clicked, self.s3_structure_sel)
			self.trans_s3_s4 = self.s3_structure_sel.addTransition(self.btn_right.clicked, self.s4_software)
			self.trans_s4_s3 = self.s4_software.addTransition(self.btn_left.clicked, self.s3_structure_sel)
			self.trans_s3_s2 = self.s3_structure_sel.addTransition(self.btn_left.clicked, self.s2_structure_workflow)
			self.trans_s2_s1 = self.s2_structure_workflow.addTransition(self.btn_left.clicked, self.s1_mount_setup)
			self.trans_s1_s0 = self.s1_mount_setup.addTransition(self.btn_left.clicked, self.s0_welcome)

			# Add states
			self.state_machine.addState(self.s0_welcome)
			self.state_machine.addState(self.s1_mount_setup)
			self.state_machine.addState(self.s2_structure_workflow)
			self.state_machine.addState(self.s3_structure_sel)
			self.state_machine.addState(self.s4_software)
			self.state_machine.setInitialState(self.s0_welcome)

			# Connections
			self.s0_welcome.entered.connect(self.on_s0_welcome_entered)
			self.s1_mount_setup.entered.connect(self.on_s1_mount_setup_entered)
			...
			self.s4_software.entered.connect(self.on_s4_software)
		...
		elif self.setup == STRUCTURE:
			self.s2_structure_workflow.addTransition(self.btn_right.clicked, self.s3_structure_sel)
			self.s3_structure_sel.addTransition(self.btn_right.clicked, self.s4_software)
			self.s4_software.addTransition(self.btn_left.clicked, self.s3_structure_sel)
			self.s3_structure_sel.addTransition(self.btn_left.clicked, self.s2_structure_workflow)

			self.state_machine.addState(self.s2_structure_workflow)
			self.state_machine.addState(self.s3_structure_sel)
			self.state_machine.addState(self.s4_software)
			self.state_machine.setInitialState(self.s2_structure_workflow)
			# Structure setup
			self.s2_structure_workflow.entered.connect(self.on_s2_structure_workflow_entered)
			self.s3_structure_sel.entered.connect(self.on_s3_structure_sel_entered)
			self.s4_software.entered.connect(self.on_s4_software)

			self.s2_structure_workflow.assignProperty(self.btn_left, "text", "Cancel")
			self.s2_structure_workflow.assignProperty(self.btn_right, "text", "Next")
			self.s3_structure_sel.assignProperty(self.btn_left, "text", "Back")
			self.s3_structure_sel.assignProperty(self.btn_right, "text", "Next")
			self.s4_software.assignProperty(self.btn_left, "text", "Back")
			self.s4_software.assignProperty(self.btn_right, "text", "Finish!")
		...
		if setup == FULL:
			self.steps_layout.addWidget(self.cb_s0_welcome, 0, QtCore.Qt.AlignCenter)
	...
	def on_le_mount_text_changed(self, text):
		...
		if os.path.exists(text):
			...
			if os.path.exists(shared_path):
				...
				self.s1_mount_setup.removeTransition(self.trans_s1_s2)
				...
				# Add new transitions
				self.trans_s1_s4 = self.s1_mount_setup.addTransition(self.btn_right.clicked, self.s4_software)
				self.trans_s4_s1 = self.s4_software.addTransition(self.btn_left.clicked, self.s1_mount_setup)
				...
				if self.setup == MOUNT:
					...
					# Remove old transitions
					self.s1_mount_setup.addTransition(self.trans_s1_s2)
					...
					# Add new transitions
					self.s1_mount_setup.removeTransition(self.trans_s1_s4)
	...
	def on_s0_welcome_entered(self):
		# Steps
		self.cb_s0_style = """
		QCheckBox::indicator:checked:disabled {{
			image: url({0}/resources/icon/checkbox_unchecked.svg);
		...
		""".format(self.armada_root_path)
		self.cb_s0_welcome.setChecked(True)
		self.cb_s0_welcome.setStyleSheet(self.cb_s0_style)
	...
	def on_s1_mount_setup_entered(self):
		# Steps
		self.cb_s1_style = """
		QCheckBox::indicator:checked:disabled {{
			image: url({0}/resources/icon/checkbox_unchecked.svg);
		...
		""".format(self.armada_root_path)
		self.cb_s0_welcome.setChecked(False)
		self.cb_s0_welcome.setStyleSheet(self.cb_s0_style)
	...
	def on_accept_pressed(self):
		...
		if self.setup == FULL or self.setup == STRUCTURE:
			...
			make_dirs(structures_data_root_path)
